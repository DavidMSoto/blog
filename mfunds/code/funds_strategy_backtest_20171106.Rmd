...
# lets gather a bit more information 
...
http://tools.morningstar.co.uk/api/rest.svc/t92wz0sj7c/security_details/F000001F03?viewId=sustainability&idtype=msid&responseViewFormat=json&languageId=en
...
REC.data <-getURL(url_get) %>% fromJSON()  %>% as.data.frame


if (ncol(asta) > 5 ) # these are the products with ESG information
{

  df.return <-rbind( df.return ,REC.data )
 
   } else {   print( paste0( "the product",  MFEIInc[i,5], "Does not have sustianibilita dadta")) }
